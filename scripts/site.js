...
Y.use('squarespace-gallery-ng', 'squarespace-fullscreen', function(Y) {
  ...
  var FULL_VERSION_MIN_WIDTH = 640,
      ...
      if (!fullscreen) {
        var setHeight = function() {
          Y.one('.slideshow').setStyles({
            height: Y.config.win.innerHeight - Y.one('#header').height(),
    ...
    function hideInfo(callback) {
      if (!isMobileEnabled()) {
        ...
        $images.each(function(image) {
          ...
          var imageSize = ImageLoader.getSquarespaceSize(winWidth, winHeight, {
            dimensions: getDimensionsFromNode(image)
          });

          // load higher resolution version one if needed
          if (imgSrc && image.getAttribute('data-image-dimension') !== imageSize) {
            image.setAttribute('src', image.getAttribute('data-src') + '?format=' + imageSize);
          }
  ...
  function scrollTo(element, duration, callback) {
    ...
    var anim = new Y.Anim({ node: scrollNodes, to: { scroll: position }, duration: duration, easing: Y.Easing.easeInOutExpo});
